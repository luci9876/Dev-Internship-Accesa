...
const TrainerProfile: React.FC<UserProfileProps<Trainer>> = ({ user }) => {
    const [userInfo, setUserInfo] = useState<Trainer>(user);
    ...
    const updateProfileInfo = async () => {
        if (formValid) {
            setIsLoading(true);
            await TrainerService.updateTrainer(userInfo);
            setIsLoading(false);
            history.go(0);
    ...
    const downgradeToTraineeProfile = async () => {
        userInfo.role = {
        ...
        if (formValid) {
            await TrainerService.updateTrainer(userInfo);
            await TrainerService.deleteTrainer(userInfo.trainerId);
                ...
                : <div className="sb-user-profile-full">
                  <div className="sb-user-profile">
                    <div className="sb-profile-title">
                        ...
                        <div className="sb-user-profile-rating">
                            {userInfo.rating}
                        </div>
                    </div>
                    ...
                    <form className="sb-user-form">
                        ...
                        <div className="sb-profile-buttons">
                            ...
                            <PrimaryButton
                                text={intl.formatMessage({ id: "app.userProfile.btn.Delete" })}
                                disabled={!formValid}
                                className="sb-extended-challenge-buttons-delete"
                                onClick={downgradeToTraineeProfile} />
                        </div>
                    </form>
                </div>
            </div>
