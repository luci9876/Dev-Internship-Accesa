...
class JoinEventService {
    ...
    public static getAllEvents = async (): Promise<Response<EventExtended[]>> => {
        try {
            const result = await http.get(`${APIRoutes.SportsBuddyAPI}/JoinEvent`, { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } });
            return {
                data: result.data,
                hasError: false,
            }
    ...
    public static getEventById = async (userId: number, eventId: number): Promise<Response<Event>> => {
        try {
            const result = await http.get(`${APIRoutes.SportsBuddyAPI}/JoinEvent/${userId}/${eventId}`, { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } });
            return {
                data: result.data,
                hasError: false,
            }
    ...
    public static addEvent = async (event: JoinEvent): Promise<Response<Event>> => {
        try {
            const result = await http.post(`${APIRoutes.SportsBuddyAPI}/JoinEvent`, event, { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } });
            return {
                data: result.data,
                hasError: false,
            }
    ...
    public static deleteEvent = async (userId: number, eventId: number): Promise<Response<Event>> => {
        try {
            const result = await http.delete(`${APIRoutes.SportsBuddyAPI}/JoinEvent/${userId}/${eventId}`, { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } });
            return {
                data: result.data,
                hasError: false,
            }
